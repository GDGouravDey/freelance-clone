...
const getAllAppliedOffer = asyncHandler(async (req,res) => {
    if (req.user.role !== 'employee') {
        return res.status(403).json({ message: 'Only employees can complete tasks', success: false })
    }

    const employeeId = req.user?._id
    const applications = await Application.find({ applicant: employeeId }).populate('freelancingOffer', 'title description').populate('applicant', 'username')

    return res.status(200).json(new ApiResponse(200, applications, 'All applied offers fetched successfully'))
})
...
export { applyForOffer, acceptApplication, rejectApplication, completeTask, getAllAppliedOffer };