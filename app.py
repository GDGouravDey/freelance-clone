...
def calculate_project_match_score(extracted_skills, project_skills):
    matched_skills = set(extracted_skills).intersection(set(project_skills))
    match_score = (len(matched_skills) / len(project_skills)) * 100
    return match_score
...
def upload_file():
    ...
    if file and file.filename.endswith('.pdf'):
        ...
        for pid, info in projects.items():
            project_match_score = calculate_project_match_score(matched_skills, info['skills_required'])
        ...
        # Sort by skill match score first, then by budget
        # Sort in reverse order for match_score to ensure highest scores are at the top
        project_scores.sort(key=lambda x: (-x['match_score'], x['budget']))
